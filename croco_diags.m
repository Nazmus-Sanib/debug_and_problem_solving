...
%
    ...
    var = (u2rho_3d(nc{'u'}(:,1:74,1:80,n))).^2+(u2rho_3d(nc{'v'}(:,1:74,1:80,n))).^2; 
    %disp(size(var))
%
% 2: Kinetic Energy
%
        %var=(u2rho_3d(nc{'u'}(n,1:74,1:80,1))).^2+(v2rho_3d(nc{'v'}(n,1:74,1:80,1))).^2;
        ...
        avgke(tndx)=sum(sum(sum(var.*dv(:,1:70,:))))/V(tndx);
...
%
        ...
        for ii = 1:70
          for jj = 1:77
            ...
            avgzeta(tndx)=sum(sum(var.*DS2D(ii,jj)))/S;
          end
        end  
